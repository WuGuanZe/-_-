...
%%找質心 為種子點 
%----------------------------------------------------------------------
sum_x = 0;  sum_y = 0;  sum_z = 0; area = 0; 
    for i = 1:xdim   
        for j = 1:ydim
            for k = 1:zdim             
              if diff_image_th_close(i,j,k) == 1
                sum_x = sum_x+i; 
                sum_y = sum_y+j;  
                sum_z = sum_z+k; 
                area = area+1;   
    ...
    mean_x = fix(sum_x/area); 
    mean_y = fix(sum_y/area); 
    mean_z = fix(sum_z/area);

seed_x = mean_x;
seed_y = mean_y;
seed_z = mean_z;
str=sprintf('The seed point is (%d,%d,%d)', seed_x,seed_y,seed_z);
disp(str);
...
for i=1:7
    for j=1:7
        for k=1:7
            sp_region(i,j,k)=region_image(seed_x+i-2,seed_y+j-2,seed_z+k-2);
        end
    end
end
...
%%確定種子點是否正確
%----------------------------------------------------------------------
region_image(seed_x+max_x,seed_y+max_y,seed_z+max_z)
region_image(seed_x+min_x,seed_y+min_y,seed_z+min_z)
region_image(seed_x+med_x,seed_y+med_y,seed_z+med_z)
