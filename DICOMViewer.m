...
% Last Modified by GUIDE v2.5 19-Dec-2012 16:47:13
...
%         seq = metadata.SequenceName;
       ...
       % newname = ['pid' pid '-seq' seq '-image' num2str(b) '-TE' num2str(TE)  '-TR' num2str(TR) '.dcm'];
         newname = [num2str(b) '.dcm'];
         %newname = [num2str(c) '.dcm'];
        % --- create the a new directory with the series #
        wiam = cd;
        cd(handles.dfolder)
       % dirname = ['PatientID' pid '-series' num2str(ser)];
       dirname = ['series' num2str(ser)];
       a = ['D:\' pid '\' SD ]
        if exist([handles.dfolder '\' dirname],'dir')~=7,
            mkdir(a,dirname)
        end
        destination = ['D:\' pid '\' SD '\' dirname '\' newname];
        %destination = ['D:\' pid '\' SD '\' dirname '\' newname2];
